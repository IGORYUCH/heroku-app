from flask import Flask, request, make_response, jsonify, redirect, url_for, render_template, send_file, make_response as fresponse
...
@app.route("/")
def teapot():
    lst = ["Your cock is about {0} inches", "Your iq is about {0} points", "You are gay about {0} percents"]
    mark = ["Not bad not terrible", "Very well", "Could be worst"]
    return f"{choice(lst).format(randint(1,120))}. {choice(mark)}", 418
...
def proxy():
    if request.args.get("url", False):
        ...
        url = parse.unquote(request.args["url"])
        result = get(url, headers=headers)
        response = fresponse(result.content)
        response.headers["Content-Type"] = result.headers["Content-Type"]
        return result.content
    else:
        return "no url performed", 400
